...
def otpravka_soob():
    ...
    while True:
        ...
        if len(buffer_soob_otpavit) != 0:
            soob_bufer = buffer_soob_otpavit[0][0]
            vremya_buffer = buffer_soob_otpavit[0][2]
            soob_pol_buffer = buffer_soob_otpavit[0][3]
            response = requests.get(str("http://"+ip+"/"+"b"+"*"+token+"*"+str(soob_pol_buffer)+"*"+soob_bufer))
    ...
    for i in range(len(papka)):
        if os.path.isfile(papka[i]):
            ...
            for k in range(len(soob)):
                if soob[k][3] == 0: 
                    buffer_soob_otpavit.append([soob[k][0], soob[k][1], soob[k][2], papka[i]])
...
izmenenie = True
#Загрузка необхлдимых элементов для главного экран
                ...
                if event.ui_element == otpravit_okno_soob:
                    ...
                    if vvod_soob_okno_soob.get_text() != "":
                            ...
                            if len(soob_vybran[soob_pol]) == 1:
                                ...
                                if soob_pol != "15421":
                                    buffer_soob_otpavit.append([base64.b64encode(bytes(vvod_soob_okno_soob.get_text().encode('utf-8'))).decode("utf-8").replace('DQ==', '', 0), "0", time.time(), soob_pol])
                        ...
                        if soob_vybran[soob_pol] != []:
                            for i in range(len(soob_vybran[soob_pol])):
                                for j in range(len(soob)):
                                    if soob[j][2] == soob_vybran[soob_pol][i]:
                                        soob.pop(j)
                                        soob_vybran[soob_pol][i] = -1
                                        break
                            while -1 in soob_vybran[soob_pol]:
                                soob_vybran[soob_pol].remove(-1)
                        ...
                        if soob_vybran[soob_pol] != []:
                            for i in range(len(soob_vybran[soob_pol])):
                                for j in range(len(soob)):
                                    if soob[j][2] == soob_vybran[soob_pol][i]:
                                        soob.pop(j)
                                        soob_vybran[soob_pol][i] = -1.0
                                        break
                            while -1.0 in soob_vybran[soob_pol]:
                                soob_vybran[soob_pol].remove(-1.0)
